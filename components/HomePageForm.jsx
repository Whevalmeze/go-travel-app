...
const HomePageForm = ({page, checkInDate, checkOutDate, date, location, setLocation, setCheckInDate, setCheckOutDate, setSearchQuery}) => {
  ...
  return (
    <form className="flex justify-between absolute bottom-[-60px] bg-amber-500 p-1 text-black min-w-full rounded-md gap-1">
        ...
        <input type="date" value={checkInDate} min={date}  className="bg-[#fff] hidden md:block cursor-pointer w-[25%] py-2 px-4 border rounded h-full" onChange={(e)=> setCheckInDate(e.target.value)} name="checkindate" id="" />
        <input type="date" name="checkoutdate" min={date}  className="bg-[#fff] hidden md:block cursor-pointer w-[25%] py-2 px-4 border rounded h-full" value={checkOutDate} onChange={(e)=> setCheckOutDate(e.target.value)} id="" /> 
        <input type="submit" value="Search" className="bg-[#003b95] w-[auto] cursor-pointer rounded px-6 py-2 text-white" onClick={(e)=> handleSubmit(e)} />
    </form>  )
}
